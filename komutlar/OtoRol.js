...
exports.run = async (client, message, args) => {
  ...
  if (args[0] === "rol-ayarla") {
    ...
    if (!rol) return message.channel.send(
      new Discord.MessageEmbed()
        .setColor("BLACK")
        .setDescription(`Bir Rol İsmi | ID'si Girmelisin \n **Örnek Kullanım** » \n \`${prefix}otorol rol-ayarla @「♦️」Oyuncu\` `)
       .setTimestamp()
    ...
    if (!kanal) return message.channel.send("Bir kanal etiketlemediniz");
    else {
    ...
    if (!db.has(`${message.guild.id}_otokanal`))
      return message.channel.send("Zayen otorol kanal ayarlanmamış");
    else {
